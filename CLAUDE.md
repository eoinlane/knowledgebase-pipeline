...
5. **Trust threshold (added 2026-06-14):** a matched event is only used for `attendees` when its raw cost ≤ 0 (i.e. at least one bonus signal — voice overlap, title cue, small attendee count — has applied beyond the bare delta penalty). Below that the frontmatter still emits `matched_event` / `matched_event_score` / `matched_event_delta_min` for the audit trail, plus `match_trusted: false`, but `attendees_source` becomes `csv:key_people` (LLM key_people) or `none`. Pre-threshold ~50% of KB meetings (374/749) carried a `calendar` source that was a pure best-delta guess with no semantic signal — the matcher had glued one "OpenAI Frontiers" conference event to 5 different unrelated recordings 6–8h away, "AI Strategy and Governance" recordings to "Thurs Scouts Troop meeting", etc. Morning brief surfaces last-7-day weak/no-match meetings so the user can correct via `kb_corrections.json` before the bad attribution propagates into syntheses + people graph.
...
1. **`identify_speakers.py:auto_enrol()`** — extends existing catalog entries: appends the recording's embedding when a voice matches a known person. **Adaptive threshold (2026-06-14):** strict 0.92/0.10 (sim/margin, high-confidence only) for entries with ≥4 samples — protect a sturdy profile. Relaxed 0.75/0.10 (medium-confidence accepted) for ≤3-sample entries — singletons need samples more than the catalog needs strictness. Pre-2026-06-14 the static 0.92 gate fired so rarely (a second 1-on-1 typically scores 0.78–0.88, not 0.92+) that 31 of 49 catalog people were singletons and 45 of 49 were ≤3 samples. Margin requirement (0.10) is the safety net — strict separation from runner-up keeps the relaxed sim gate from poisoning a profile. Compounds reliability over time. Runs inline at speaker_id step.
